...
const fileName = 'openReserveLocker'; // Nombre del archivo para los logs
...
const OpenReserveLocker = async (payload) => {
    log('info', 'Iniciando petición para abrir casillero reservado');
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            log('info', `Intento ${attempt}: URL -> ${API_ROUTES.OPEN_RESERVE_LOCKER}`);
            log('info', `Intento ${attempt}: Request -> ${JSON.stringify(payload)}`);

            const response = await axios.post(API_ROUTES.OPEN_RESERVE_LOCKER, payload, { timeout: effectiveTimeout });
...
export default OpenReserveLocker;
