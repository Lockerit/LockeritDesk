...
export const SetStatusLocker = async (payload) => {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            log('info', `Intento ${attempt}: URL -> ${API_ROUTES.SET_STATUS_LOCKER}`);
            log('info', `Intento ${attempt}: Request -> ${JSON.stringify(payload)}`);

            const response = await instanceAxios.post(API_ROUTES.SET_STATUS_LOCKER, payload, { timeout: effectiveTimeout });
