...
export const Reserve = async (payload) => {
    ...
    const url = API_ROUTES.RESERVE_LOCKERS;

    log.info(`Petición reserve, { baseURL: ${instanceAxios.defaults.baseURL}, url: ${url}, timeoutMs: ${effectiveTimeout}, maxRetries: ${maxRetries}, payload: ${JSON.stringify(redactPayload(payload), null, 2)} }`);
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            // Reintentar: 5xx, 429 o fallo de red (status null)
