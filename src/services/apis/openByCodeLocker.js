...
const fileName = 'openByCodeLocker'; // Nombre del archivo para los logs
...
export const OpenByCodeLocker = async (payload) => {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            log('info', `Intento ${attempt}: URL -> ${API_ROUTES.OPEN_BY_CODE_LOCKER}`);
            log('info', `Intento ${attempt}: Request -> ${JSON.stringify(payload)}`);

            const response = await instanceAxios.post(API_ROUTES.OPEN_BY_CODE_LOCKER, payload, { timeout: effectiveTimeout });
